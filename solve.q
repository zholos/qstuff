...
1"LUP:       ";
\t x3:bsub[LUP 1;fsub[LUP 0;b(LUP:.qml.mlup A)2]];

/ solve by Cholesky factorization
/   Ax=b  =>  A'Ax=A'b  <=>  R'Rx=A'b  <=>  R'y=A'b, Rx=y (R is triangular)
1"Cholesky:  ";
\t x4:bsub[R;fsub[flip[R:.qml.mchol A_ mmu A];(A_:flip A)mmu b]];

/ solve by singular value decomposition
/   Ax=b  <=>  USV'x=b  =>  x=V inv(S)U'b (S is diagonal)
1"SVD:       ";
\t x5:SVD[2]mmu(flip[SVD 0]mmu b)%(SVD:.qml.msvd A)[1]@'til n;

/ check results
if[1e-7<{max -1+(b%x)|x%b}A mmu x1;'`incorrect];
if[1e-5<max{max -1+(x1%x)|x%x1}each(x2;x3;x4;x5);'`different];
